...
bool IsInShadow(vec3 startPoint, vec3 lightDir, vec3 lightPos)
{
    ...
    {
        float progress = (float(i) / float(numOfIterations)) * maxDistance;

        vec3 point = startPoint + lightDir * progress;
...
void main()
{
    ...
    if(!IsInShadow(FragPos + offsetByNormal, lightDir, lightPos)) 
    {
