...
public class Scene
{
    readonly List<BufferData> _bufferDatas = [];

    readonly List<Vector3> _centres =
    [
        new(0f, 0, 0),
        new(0.5f, -0.5f, 0),
    ];

    readonly List<float> _sizes =
    [
        0.2f,
        0.3f,
    ];

    readonly List<Vector3> _colors =
    [
        new(0, 0, 1),
        new(0, 1, 0),
    ];
    ...
    {
        _bufferDatas.AddRange(CreateCubes());
        _bufferDatas.AddRange(CreateFloor());
    }
    ...
    public void Paint()
    {
        foreach (BufferData bufferData in _bufferDatas)
        {
